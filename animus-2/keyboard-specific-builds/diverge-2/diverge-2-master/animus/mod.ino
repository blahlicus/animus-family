...
#define builder_mpresscoord I2CPressCoord(x, y); mediaPressCoord(x, y); macroPressCoord(x, y); dualRolesPressCoord(x, y); tapDancePressCoord(x, y); mouseKeysPressCoord(x, y); stickyKeysPressCoord(x, y);
#define builder_mprepress I2CPrePress(val, type); mediaPrePress(val, type); macroPrePress(val, type); dualRolesPrePress(val, type); tapDancePrePress(val, type); mouseKeysPrePress(val, type); stickyKeysPrePress(val, type);
